...
if __name__=='__main__':
    ...
    print(f'Optimizing using cpp conv2d model')
    assert(device == 'cpu')
    cpp_filter_model = CppEdgeFilter().to(device)
    optimize_for_weights(image.clone(), edgeImage.clone(), cpp_filter_model, verbose=True)

    print(f'Optimizing using torch model')
    torch_filter_model = TorchEdgeFilter().to(device)
    optimize_for_weights(image.clone(), edgeImage.clone(), torch_filter_model, verbose=True)
